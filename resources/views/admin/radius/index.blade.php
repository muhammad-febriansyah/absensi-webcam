                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body table-responsive">
                            <a href="{{ route('main.createradius') }}" class="btn btn-primary mr-3 mb-3"><i class="fa fa-plus"
                                    aria-hidden="true"></i> Form radius Penempatan</a> <a href="{{ route('main.radius') }}"
                            ...
                            <table class="table table-bordered table-hover" id="dt">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $data)
                                        <tr>
                                            <td>{{ $no++ }}</td>
                                            <td>{{ $data->lokasiPenempatan->name }}</td>
                                            <td>{{ $data->lat }}, {{ $data->long }}</td>
                                            <td>{{ $data->radius }}</td>
                                            <td>
                                                <a href="{{ route('main.editradius', $data->id) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete', function() {
                    var id = $(this).data("id");
                    var url = "{{ route('main.destroyradius') }}";
                    Swal.fire({
